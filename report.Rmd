...
```{r eda_ctr_by_group_enwiki_only, results = 'asis'}
sessions %>%
  ...
  prop.table(margin = 1) %>%
  knitr::kable(caption = "Proportion of sessions that clicked specifically on an English Wikipedia result vs proportion that did not click on any result. The two test groups ('b' and 'c') have been combined into a single group.")
```

```{r cda_ctr_by_group_enwiki_only, results = 'asis'}
set.seed(0)
test_1 <- sessions %>%
  mutate(any_enwiki_clicks = ifelse(any_enwiki_clicks, "Clicked an enwiki result", "Did not")) %>%
  group_by(group = ifelse(test_group == "a (control)", "controls", "test"),
...
sessions %>%
  ...
  prop.table(margin = 1) %>%
  knitr::kable(caption = "Proportions of clicks on results (enwiki and interwiki). The two test groups ('b' and 'c') have been combined into a single group.")
```
...
sessions %>%
  ...
  xtabs(n ~ group + clickthrough, data = .) %>%
  prop.table(margin = 1) %>%
  knitr::kable(caption = "Proportion of sessions that clicked on a result vs that did not click on any result. For the groups 'b' and 'c', these clickthrough rates account for both types of results that could be clicked -- enwiki and interwiki.")
```
...
searches %>%
  ...
  group_by(interwiki = ifelse(zero_interwiki_results, "zero interwiki results", "some interwiki results"),
           enwiki = ifelse(zero_enwiki_results, "zero enwiki results", "some enwiki results")) %>%
  tally() %>%
  xtabs(n ~ interwiki + enwiki, data = .) %>%
  add_props %>%
  knitr::kable(caption = "Counts and proportions of searches with some or no enwiki and interwiki results where we detected a language and the user clicked on a result.")
```
...
results <- searches %>%
  ...
  mutate(`enwiki results` = results_returned - new_results_returned) %>%
  group_by(`enwiki results`, first_click_position, `first clicked on` = ifelse(first_click_is_interwiki, "an interwiki result", "an enwiki result")) %>%
  tally() %>%
  ungroup %>%
  mutate(first_click_position = ifelse(first_click_position <= 3, first_click_position, 4)) %>%
  group_by(`enwiki results`, `first clicked on`, first_click_position) %>%
  summarize(n = sum(n)) %>%
...
knitr::kable(results, caption = "Do people just click on the first link even if the better result is the interwiki result that's lower in the list? Seems like it.")
```
...
present_bbfit(list("Clicked on an enwiki result" = test_1,
